...
if (isset($_POST['delete'])) {
    $deleteSql = "DELETE FROM tbusuario WHERE cpf = '$cpf'";
    $deleteQuery = mysqli_query($mysql->con, $deleteSql);

    if ($deleteQuery) {
        echo "<script language='javascript' type='text/javascript'>
          alert('Usuario foi removido com sucesso.');window.location.href='controle_usuario.php';
          </script>";
        exit;
    } else {
        echo "<script language='javascript' type='text/javascript'>
          alert('Ops, tivemos um erro ao remover.');window.location.href='controle_usuario.php';
          </script>";
        ;
    }
}
            ...
            <div class="col-xl-8 col-lg-7">
                <div class="card shadow mb-4">
                    ...
                    <div class="card-header py-3 d-flex flex-row align-items-center justify-content-center">
                        <h6 class="m-0 font-weight-bold text-primary">Excluir Usuario</h6>
                        <div class="dropdown no-arrow">
                        </div>
                    </div>
                    <center>
                        <h1>Excluir Usuario</h1>
                        <h3>Tem certeza de que deseja excluir o usuario:
                        <span style="color: #870101FF; font-weight: bold;""> <?php echo $partnerData['nome']; ?>?</span>
                        </h3>
                        <form method="POST" action="">
                            <input class="btn btn-primary" type="submit" name="delete" value="Excluir">
                            <a class="btn btn-primary" href="controle_usuario.php">Cancelar</a>
                        </form>
                    </center>
